...
if ( ! class_exists( 'Webbox_Featured_Posts_Block_Widget' ) ) {
	 ...
	 * @since 1.0.0
	 */
	class Webbox_Featured_Posts_Block_Widget extends Cherry_Abstract_Widget {
		...
		public function __construct() {
			$this->widget_name        = esc_html__( 'Featured Posts Block', 'webbox' );
			$this->widget_description = esc_html__( 'This widget displays latest posts', 'webbox' );
			$this->widget_id          = 'webbox_widget_featured_posts_block';
			...
			foreach( $layouts as $id => $layout ) {
				$layout_options[ $id ] = array(
					'label'   => $layout['name'],
					'img_src' => WEBBOX_THEME_URI . '/assets/images/admin/widgets/featured-posts-block/' . $id . '.svg',
				);
			}
			...
			$this->settings = array(
				...
				'posts_ids' => array(
					'type'  => 'text',
					'value' => '',
					'label' => esc_html__( 'Posts IDs (Optional)', 'webbox' ),
				),
				'checkboxes'     => array(
					'type'  => 'checkbox',
					'label' => esc_html__( 'Post Meta', 'webbox' ),
					'value' => array(
						'title'      => 'true',
						'excerpt'    => 'true',
						'categories' => 'true',
					...
					'options' => array(
						'title'      => esc_html__( 'Show title', 'webbox' ),
						'excerpt'    => esc_html__( 'Show excerpt', 'webbox' ),
						'categories' => esc_html__( 'Show categories', 'webbox' ),
						'tags'       => esc_html__( 'Show tags', 'webbox' ),
						'author'     => esc_html__( 'Show author', 'webbox' ),
						'date'       => esc_html__( 'Show date', 'webbox' ),
					),
				),
				'title_length' => array(
					'type'      => 'stepper',
					'value'     => 12,
					'min_value' => 1,
					'label'     => esc_html__( 'Title length (chars)', 'webbox' ),
				),
				'excerpt_length' => array(
					'type'      => 'stepper',
					'value'     => 15,
					'min_value' => 1,
					'label'     => esc_html__( 'Excerpt length (words)', 'webbox' ),
				),
			);
		...
		public function render_layout( $options = array() ) {
			...
			$item_template = locate_template(  apply_filters( 'webbox_featured_posts_block_widget_view_dir', 'inc/widgets/featured-posts-block/views/item.php' ), false, false );
			 ...
			 */
			$query = apply_filters( 'webbox_featured_posts_block_query', $query, $this->instance );
	...
	add_action( 'widgets_init', 'webbox_register_featured_posts_block_widget' );

	if ( false === function_exists( 'webbox_register_featured_posts_block_widget' ) ) {
		/**
		 * Register featured posts block widget.
		 */
		function webbox_register_featured_posts_block_widget() {
			register_widget( 'Webbox_Featured_Posts_Block_Widget' );
		}
	}
}
