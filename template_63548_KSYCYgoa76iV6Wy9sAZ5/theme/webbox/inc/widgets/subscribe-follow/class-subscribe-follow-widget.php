...
class Webbox_Subscribe_Follow_Widget extends Cherry_Abstract_Widget {
	...
	public function __construct() {
		...
		$this->widget_description = esc_html__( 'Display subscribe form and follow links.', 'webbox' );
		$this->widget_id          = 'webbox_widget_subscribe_follow';
		$this->widget_name        = esc_html__( 'Subscribe and Follow', 'webbox' );
		$this->settings           = array(
			'enable_subscribe' => array(
				...
				'options' => array(
					'enable_subscribe' => esc_html__( 'Enable Subscribe Box', 'webbox' ),
				),
			),
			'subscribe_title' => array(
				'type'  => 'text',
				'value' => esc_html__( 'Subscribe', 'webbox' ),
				'label' => esc_html__( 'Subscribe Title', 'webbox' ),
			),
			'subscribe_message' => array(
				'type'  => 'textarea',
				'label' => esc_html__( 'Subscribe text message', 'webbox' ),
			),
			'subscribe_input' => array(
				'type'  => 'text',
				'value' => esc_html__( 'Enter your email', 'webbox' ),
				'label' => esc_html__( 'Subscribe input placeholder', 'webbox' ),
			),
			'subscribe_submit' => array(
				'type'  => 'text',
				'value' => esc_html__( 'Subscribe', 'webbox' ),
				'label' => esc_html__( 'Subscribe submit label', 'webbox' ),
			),
			'subscribe_success' => array(
				'type'  => 'text',
				'value' => esc_html__( 'You successfully subscribed', 'webbox' ),
				'label' => esc_html__( 'Subscribe success', 'webbox' ),
			),
			'enable_follow' => array(
				'type'   => 'checkbox',
				'value'  => array(
				...
				'options' => array(
					'enable_follow' => esc_html__( 'Enable Follow Box', 'webbox' ),
				),
			),
			'follow_title' => array(
				'type'  => 'text',
				'value' => esc_html__( 'Follow', 'webbox' ),
				'label' => esc_html__( 'Follow Title', 'webbox' ),
			),
			'follow_message' => array(
				'type'  => 'textarea',
				'label' => esc_html__( 'Follow text message', 'webbox' ),
			),
			'enable_background' => array(
				...
				'options' => array(
					'enable_background' => array(
						'label' => esc_html__( 'Enable Custom Background', 'webbox' ),
			...
			'background_image' => array(
				'type'               => 'media',
				'label'              => esc_html__( 'Background Image', 'webbox' ),
				'upload_button_text' => esc_html__( 'Choose Image', 'webbox' ),
				'multi_upload'       => false,
			...
			'invert_text_colorscheme' => array(
				...
				'master'  => 'background_image',
				'options' => array(
					'invert_text_colorscheme' => esc_html__( 'Use "Invert scheme" for text color', 'webbox' ),
				),
			),
			'background_position' => array(
				'type'    => 'select',
				'label'   => esc_html__( 'Background Position', 'webbox' ),
				'value'   => 'center',
				'options' => array(
					'top-left'      => esc_html__( 'Top Left', 'webbox' ),
					'top-center'    => esc_html__( 'Top Center', 'webbox' ),
					'top-right'     => esc_html__( 'Top Right', 'webbox' ),
					'center-left'   => esc_html__( 'Middle Left', 'webbox' ),
					'center'        => esc_html__( 'Middle Center', 'webbox' ),
					'center-right'  => esc_html__( 'Middle Right', 'webbox' ),
					'bottom-left'   => esc_html__( 'Bottom Left', 'webbox' ),
					'bottom-center' => esc_html__( 'Bottom Center', 'webbox' ),
					'bottom-right'  => esc_html__( 'Bottom Right', 'webbox' ),
				),
				'master' => 'background_image',
			),
			'background_repeat' => array(
				...
				'value'   => 'no-repeat',
				'options' => array(
					'repeat'    => esc_html__( 'Repeat', 'webbox' ),
					'repeat-x'  => esc_html__( 'Repeat X', 'webbox' ),
					'repeat-y'  => esc_html__( 'Repeat Y', 'webbox' ),
			...
			'background_size' => array(
				'type'    => 'select',
				'label'   => esc_html__( 'Background Size', 'webbox' ),
				'value'   => 'inherit',
				'options' => array(
					'cover'   => esc_html__( 'Cover', 'webbox' ),
					'contain' => esc_html__( 'Contain', 'webbox' ),
					'auto'    => esc_html__( 'Auto', 'webbox' ),
				),
				'master' => 'background_image',
			),
			'background_color' => array(
				'type'   => 'colorpicker',
				'label'  => esc_html__( 'Background Color', 'webbox' ),
				'value'  => '#000000',
				'master' => 'background_image',
			),
		);

		add_action( 'wp_ajax_webbox_subscribe', array( $this, 'process_subscribe' ) );
		add_action( 'wp_ajax_nopriv_webbox_subscribe', array( $this, 'process_subscribe' ) );

		$this->errors = array(
			'nonce'     => esc_html__( 'Security validation failed', 'webbox' ),
			'mail'      => esc_html__( 'Please, provide valid mail', 'webbox' ),
			'mailchimp' => esc_html__( 'Please, set up MailChimp API key and List ID', 'webbox' ),
			'internal'  => esc_html__( 'Internal error. Please, try again later', 'webbox' ),
		);

		$this->nonce = wp_create_nonce( 'webbox-subscribe-nonce' );
	...
	public function widget( $args, $instance ) {
		...
		$follow_template            = locate_template( apply_filters( 'webbox_subscribe_widget_follow_template_file', 'inc/widgets/subscribe-follow/view/follow-view.php' ), false, false );
		$subscribe_template         = locate_template( apply_filters( 'webbox_subscribe_widget_subscribe_template_file', 'inc/widgets/subscribe-follow/view/subcribe-view.php' ), false, false );
		...
		if ( $subscribe_enabled && $subscribe_template && $api_key && $list_id ) {
			...
			esc_html_e( 'Please set up MailChimp API key and List ID', 'webbox' );
		}
	...
	public function get_social_nav() {
		return webbox_get_social_list( 'widget' );
	}
	...
	public function get_subscribe_submit( $class ) {
		...
		$subscribe_submit = webbox_render_icons( $subscribe_submit );
	...
	public function process_subscribe() {
		...
		if ( ! isset( $_POST['nonce'] ) || ! wp_verify_nonce( $_POST['nonce'], 'webbox-subscribe-nonce' ) ) {
...
add_action( 'widgets_init', 'webbox_register_subscribe_follow_widgets' );
/**
 * Register subscribe-follow widget.
 */
function webbox_register_subscribe_follow_widgets() {
	register_widget( 'Webbox_Subscribe_Follow_Widget' );
}
