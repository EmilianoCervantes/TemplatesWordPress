 ...
 */
add_filter( 'template_include', array( 'Webbox_Wrapping', 'wrap' ), 99 );
add_filter( 'webbox_wrap_base', 'webbox_wrap_base_cpts' );
 ...
 */
function webbox_template_path() {
	return Webbox_Wrapping::$main_template;
}
 ...
 */
function webbox_template_base() {
	return Webbox_Wrapping::$base;
}

if ( ! class_exists( 'Webbox_Wrapping' ) ) {
	...
	class Webbox_Wrapping {
		...
		/**
		 * Initialize new instance of Webbox_Wrapping class by setting a variable for the $slug (which defaults to base)
		...
		public function __toString() {
			$this->templates = apply_filters( 'webbox_wrap_' . $this->slug, $this->templates );
		 ...
		 * and $base as static variables in Webbox_Wrapping class.
		 *
		 * @since 1.0.0
 ...
 */
function webbox_wrap_base_cpts( $templates ) {
