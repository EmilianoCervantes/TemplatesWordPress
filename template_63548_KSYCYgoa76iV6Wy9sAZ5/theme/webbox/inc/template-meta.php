 ...
 * @return array
 */
function webbox_share_buttons( $context = 'loop', $args = array(), $config = array() ) {
	...
	if ( ! webbox_is_meta_visible( $meta, $context ) ) {
		return;
	}
	...
	$default_config = apply_filters( 'webbox_default_config_share_buttons', array(
	...
	$share_item_html = apply_filters( 'webbox_share_button_html',
	...
	foreach ( (array) $networks as $id => $network ) :
		...
		$share_buttons .= sprintf(
			...
			esc_html__( 'Share on ', 'webbox' ) . $network['name'],
 ...
 * @return string
 */
function webbox_get_single_share_buttons() {
	ob_start();
	webbox_share_buttons( 'single' );
